...
def sql_generator(prompt: str) -> str:
    ...
    try:
        ...
        model = GenerativeModel(constants.MODEL)
...
def stock_purchase_order_generator(omni_item_id: str)-> str:
    """
    Generates stock purchase orders

                        
    Returns:
        A success message or an error description.
    """
    logger.info(f"stock_purchase_order_generator")

    try:
        rows = utils.generate_stock_purchase_order(omni_item_id)
