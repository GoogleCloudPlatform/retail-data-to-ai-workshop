...
def generate_stock_purchase_order(omni_item_id: str)-> Optional[bigquery.table.RowIterator]:
    """
    Calls the BQ stored procedure to generate a stock purchase order for a specific item
    ...
    """
    logger.info(f"Utils.py: generate_stock_purchase_order for item: {omni_item_id}")

    # Initialize the BigQuery Client
    client = bigquery.Client()
    
    # 1. Define the SQL CALL statement
    sql = f"CALL `capstone_ds.generate_stock_purchase_order`('{omni_item_id}','PROCUREMENT_MANAGER_AGENT')"
