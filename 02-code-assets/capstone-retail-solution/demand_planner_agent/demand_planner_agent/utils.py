...
def read_file_in_gcs_into_string(bucket_name: str, file_path: str) -> str:
    ...
    try:
        ...
        logger.info("INFO: Read the BQ metadata grounding file")
...
def call_adjust_forecast_stored_procedure(omni_item_id: str, adjustment_type: str) -> str:
    """
    Triggers the BigQuery stored procedure to update demand forecasts.
    ...
    logger.info("Inside call_adjust_forecast_stored_procedure")
    # Initialize the BigQuery Client
    client = bigquery.Client()
    
    # 1. Define the SQL CALL statement
    sql = f"CALL `capstone_ds.update_item_demand_forecast`('{omni_item_id}','{adjustment_type}','DEMAND_PLANNER_AGENT')"
    ...
    try:
        ...
        query_job.result() 
        logger.info(f"Successfully executed forecast update for: {omni_item_id}")
        return f"Successfully executed forecast update for: {omni_item_id}"

    except Exception as e:
        logger.error(f"Error calling BQ procedure: {e}")
        return f"Failed to update forecast. Error: {str(e)}"
    
def call_on_demand_forecast_stored_procedure() -> str:
    """
    Triggers the BigQuery stored procedure to run demand forecast for all products.

                        
    Returns:
        A success message or an error description.
    """
    logger.info("Inside call_on_demand_forecast_stored_procedure")

    # Initialize the BigQuery Client
    client = bigquery.Client()
    
    # 1. Define the SQL CALL statement
    sql = "CALL capstone_ds.run_demand_forecast('DEMAND_PLANNER_AGENT')"
    ...
    try:
        ...
        # 3. Wait for the procedure to finish
        query_job.result() 
        logger.info(f"Successfully ran demand forecast for all products")
        return f"Successfully ran demand forecast for all products"

    except Exception as e:
        logger.error(f"Error calling BQ procedure: {e}")
        return f"Failed to run on-demand forecast. Error: {str(e)}"
